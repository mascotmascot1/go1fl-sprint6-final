...
// RootHandle handles the "/" endpoint by serving "index.html"
// with http.ServeFile, which sets headers and handles errors automatically.
func RootHandle(w http.ResponseWriter, r *http.Request) {
	http.ServeFile(w, r, "index.html")

}

// UploadConvertHandle returns an http.HandlerFunc for the "/upload" endpoint.
//
...
//
// Returns http.StatusBadRequest for invalid input (e.g., no file uploaded,
// multiple files uploaded, or conversion error), and http.StatusInternalServerError
// for internal failures. Some errors are also logged using the provided logger.
func UploadConvertHandle(logger *log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err = w.Write([]byte(convertedData)); err != nil {
			logger.Printf("error writing response: %s", err)
			return
		}
	}
}
